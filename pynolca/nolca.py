...
class NOLCA:
    ...
    def training(self, X, y, learning_rate = 0.1, reg_coefficient = 0.0, unlabelled = False):
        ...
        '''
        assert y.dtype == 'int32' and ((y.min() == 0 and unlabelled == False)
                                       or (y.min() == -1 and unlabelled == True)), "Error: invalid labels." 
        ...
        labelset = set(y)
        assert (unlabelled == False and -1 not in labelset) or (unlabelled == True and -1 in labelset), "Error: ensure -1 in the set if and only if the argument `unlabelled` is `True`."
        if -1 in labelset:
            labelset.remove(-1)
        self._num_classes = len(labelset)
        assert (unlabelled == False) or (unlabelled == True and self._num_classes == 2), "Error: semi-supervised learning is allowed only in binary classification setting."
    ...
    def _update(self, x, y, learning_rate, reg_coefficient):
        ...
        # SVs, set all of the predictions to 0's. 
            ...
            pred_value = np.zeros(self._num_classes)    
        ...
        if (true_label != -1):
            ...
            loss = self._loss.loss_computing(pred_value[pred_label],
                                             ...
                                             self._num_observations,
                                             self._num_classes)
            self._accuracy.append(1 - self._get_accuracy(self._current_step + 1))  
            self._current_step += 1
            
        # Update the classifier
        gradient = None
        if (true_label != -1 and 0 < loss < 1 - self._loss._parameter) or (true_label == -1 and 0 < np.abs(pred_value[0] - pred_value[-1]) < 1):
            if isinstance(self._sample_weight, np.ndarray):
                self._sample_weight = np.hstack((self._sample_weight,
                                                 np.zeros((self._num_classes, 1))))
            else:
                self._sample_weight = np.random.normal(size = (self._num_classes, 1))
            gradient_absolute = 1 / self._kernel.compute_kernel(x, x)
            if true_label != -1 and 0 < loss < 1 - self._loss._parameter:
                gradient = gradient_absolute
            if true_label == -1 and -1 < pred_value[0] - pred_value[-1] < 0:
                pred_label, true_label, gradient = 0, 1, gradient_absolute
            if true_label == -1 and 0 < pred_value[0] - pred_value[-1] < 1:
                pred_label, true_label, gradient = 0, 1, - gradient_absolute
            self._sample_weight[pred_label, self._num_support_vectors] = \
                                            - learning_rate * gradient

            self._sample_weight[true_label, self._num_support_vectors] = \
                                              learning_rate * gradient
    ...
    def _count_num_error(self, pred_label, true_label):
        '''
        Count the number of errors at current step.
        (Internal function)
        '''
